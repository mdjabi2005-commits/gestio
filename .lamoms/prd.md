...
**Amendé une troisième et dernière fois le 2026-08-13**, par une revue d'**exécutabilité et de jugeabilité** — *Codex peut-il le faire seul, et Hermès peut-il trancher sans deviner ?* Treize décisions de plus (§11, 38 à 50) et **quinze mesures neuves**, dont celle qui manquait à tout le document : **le rejeu de l'oracle a été exécuté, et il est vert**. Un périmètre impossible est réparé, quatre recettes retrouvent un geste jouable, et le seul geste irréversible du PRD devient réversible. Aucune tâche ne change de place, aucune décision de cadrage n'est rouverte.
...
**Les libellés PDF et Enable Banking sont le même texte** — mesuré sur le dump API réel contre le relevé de juin parsé : 16 mouvements appariés, **13/16 identiques après normalisation**, les 10 écarts bruts n'étant que des espaces. Déterminer la nature par le libellé, seule trace qui survit dans un vieux relevé, **tient d'un canal à l'autre**. Les 3 écarts résiduels viennent tous d'un **débordement du parseur PDF** sur la ligne suivante — corrigé dans T31 (C12). **Et ces 3-là ne sont que la part visible d'un sous-ensemble** : mesuré sur le corpus entier le 2026-08-13, le débordement touche **24 mouvements sur les 13 relevés**, dont 20 portent une décision de l'oracle. La cause est une **borne de fenêtre** dans `lbpTransactions`, pas un mot à effacer.
...
- La suite est verte : 40 tests, lint, typecheck et build à 0. Avec le script actuel, **39 pass et 1 skip** — `node --test` ne lit pas `.env`, et le test sauté est le rejeu des 606 décisions. T31 ajoute `--env-file=.env`.
- **Le rejeu a été exécuté le 2026-08-13, et il est vert** *(décision 38 — c'est la mesure qui manquait à tout ce document)*. Commande : `node --test --env-file=.env --import tsx "src/**/*.test.ts"` dans le workspace maître. Sortie : **40 tests · 40 pass · 0 skip · 11,2 s**, le rejeu seul en **3,9 s**. Jusqu'à cette mesure, huit tâches promettaient *« l'oracle rejoue à l'identique »* sans que personne n'ait jamais vu ce à quoi il fallait rester identique. **La ligne de base existe désormais, datée** : 191 paires, 606 décisions. Conséquence pour T31 : la ligne d'outillage est **prouvée suffisante avant d'être livrée**, donc si `npm test` sort autre chose que 40/40/0 après T31, la cause est le parseur — jamais l'outillage.
...
**Le débordement de libellé** — le parseur colle à un mouvement du texte de la ligne suivante, l'un avalant `TOTAL DES OPÉRATIONS` — **plus `F6`** (le message d'erreur Trade Republic dit « compte courant » même pour un PEA) **et la ligne d'outillage**.

> **Les cinq points hérités de la review T27 ont été retrouvés et arbitrés** *(2026-08-13, décision 50)*. Ils n'étaient plus définis nulle part sur le disque : `F5` et `F6` ne vivaient que dans `.lamoms/prd-annexes.md`, archivé, et Codex les aurait reçus comme deux sigles sans contenu.
> - **M1** (un produit Trade Republic inconnu ne fait plus échouer l'import entier) et **M2** (l'IBAN du PEA vient de son segment) **partent dans T43a** : c'est mot pour mot son sujet — *« un segment non reconnu est compté et nommé, jamais sauté »*, *« `ibanFromLines` lit l'IBAN de chaque segment »*. Les laisser ici ferait toucher le même code par deux tâches à deux places de l'ordre, pour le même résultat.
> - **M3** (rattrapage des transactions Nickel importées avant T27, `qualification_label` NULL) **tombe** : il répare des lignes que la reconstruction de T45a met de côté et que le ré-import du corpus réécrit. Le PRD annexe le disait déjà — *« M3 se révèle peut-être plus large que Nickel, à mesurer avant d'écrire quoi que ce soit »* — et la mesure n'a jamais été faite. Il n'a plus d'objet.
> - **F5** (la reconnaissance du nom est indépendante de l'ordre, d'où de possibles faux positifs sur un libellé de tiers) **sort** : sa consigne d'origine était *« à documenter ou resserrer ; #40 étape 8 cherche un cas réel »*, et l'étape 8 n'a jamais été atteinte. **Aucun cas n'a été observé** — resserrer une reconnaissance sans contre-exemple, c'est deviner. Il redevient ce qu'il était : un point de **récolte** de l'étape 8, dans la recette de T35 et dans T37.
> - **F6** reste : c'est un message d'erreur dans `src/pdf-import.ts`, exactement dans le périmètre.
...
**Le débordement est huit fois plus large qu'on ne l'écrivait** *(mesuré le 2026-08-13, décision 39)*. Ce n'étaient pas « 3 écarts résiduels sur juin » : ces 3-là sont le reste d'un sous-ensemble — les 16 mouvements de juin appariés au dump API. Sur le corpus entier, **24 mouvements** portent `TOTAL DES OPÉRATIONS` dans leur libellé, répartis sur **les 13 relevés LBP**, et le texte parasite est dans `label` **et** dans `qualification_label` — donc dans ce que lit le moteur. **20 de ces 24 portent une des 606 décisions** : 10 `virement_interne` (donc des jambes de paires), 5 `virement_a_verifier`, 5 `externe_probable`. Ce n'est pas un défaut d'affichage.

**Périmètre** *(ajouté le 2026-08-13 — la tâche n'en avait aucun, seule du lot)* : **`src/pdf-import.ts` et `package.json`, et rien d'autre**. Le script `test` reçoit `--env-file=.env`. **Ne pas toucher** `src/qualification.ts`, `src/schema.ts`, `src/server.ts`, `web/src/main.jsx`, ni aucun fichier de test — `src/qualification-oracle.test.ts` est **intouchable** (décision 43), et `src/pdf-import.test.ts` n'assère de libellé que sur Trade Republic, format que cette tâche ne touche pas. **Si une assertion existante devait quand même changer, la tâche s'arrête et le signale** : ce serait le signe que la coupe a mangé autre chose que le débordement.

**La cause est trouvée, et elle n'est pas un mot** *(mesuré le 2026-08-13 dans `lbpTransactions`)* : le libellé est fabriqué en ramassant toutes les lignes situées entre le mouvement et le **suivant** ; pour le **dernier** d'un bloc, il n'y a pas de suivant, et le code retombe sur une **fenêtre fixe de 35 points** qui avale ce qui suit — le pied du tableau. D'où **deux mouvements par relevé** en moyenne, toujours les derniers. La correction ne consiste donc pas à chasser une chaîne de caractères : c'est la **borne de la fenêtre** qui est fausse.

**Critères** *(refondus le 2026-08-13, décision 39 — la version précédente se satisfaisait d'un effacement)* — (1) **les 24 mouvements sont relevés fichier par fichier, libellé avant et après**, et chacun garde **son propre texte entier** : la coupe tombe là où commence le texte étranger, elle ne mange pas un mot avant. **Cette liste reste locale** — le dépôt est public : ce qui remonte dans un commit, une issue ou un rapport, c'est **le décompte et la position de coupe**, jamais le texte d'un libellé ; (2) le rejeu sort toujours **191 paires et 606 décisions** — c'est lui qui prouve que la coupe est au bon endroit, une coupe trop courte déplaçant la qualification (simulé le 2026-08-13 : la troncature au débordement laisse le rejeu vert, 0 écart de décision, 0 écart de paire) ; (3) le décompte de mouvements par document ne bouge pas, `expected_raw_movements` pour chacun des 26 ; (4) `npm test` sort à **40 pass, 0 skip**.

> **Ce qui n'entre pas dans cette tâche** : la comparaison libellé à libellé du corpus contre l'artefact d'oracle rend **237 divergences** — 172 Trade Republic, qui ne fournit aucun libellé (C7), et **65 sur LBP/Nickel**, dont ces 24. Les 41 autres ne sont **pas un défaut établi** : on ne sait pas, sans mesure, laquelle des deux chaînes dit vrai. C'est une mesure à faire, pas un travail à commander — hors périmètre de T31.

*Recette (réécrite le 2026-08-13, décision 40)* : importer le relevé du mois — **un fichier, le geste qui a réussi le 2026-08-04** — puis, sur l'écran des mouvements, retrouver un des 24 mouvements corrigés et **lire son libellé** : il s'arrête où il doit. *Le décompte des trois natures a quitté cette recette pour celle de T35* : aucun écran ne compte par nature avant T35, et la liste s'arrête à 100 lignes — le geste était injouable ici. *L'import du lot entier appartient à T43b* : à ce stade les comptes sont encore homonymes et l'association en masse est le mur du 2026-08-11.

*Préserve* : aucune transaction supprimée, aucune ligne existante réécrite en base, l'oracle rejoue à l'identique — **191 paires, 606 décisions, 0 skip**, mesuré vert avant la tâche comme après.
...
*Recette (réécrite le 2026-08-13, revue deux fois le même jour)* : **créer un compte par l'interface en tapant son IBAN** (étape 2), puis synchroniser (étapes 3 à 5), puis **retrouver les quatre Revolut à l'écran et vérifier qu'on les distingue** — par leur IBAN pour le compte principal, par leur absence d'IBAN et leur type pour les trois pockets. *Le renommage a quitté cette recette pour celle de T45a* (décision 40) : aucun écran n'offre de renommer un compte avant l'étape de nommage de l'onboarding, et une recette ne demande que des gestes qui existent le jour où on la joue. La recette précédente ne testait que la synchro, alors que la tâche touche aussi la saisie — et c'est la saisie qui a bloqué le 2026-08-11. **Elle constate au passage ce que Revolut rend comme IBAN** — aucun verdict ne s'y accroche : la décision 28 tient quelle que soit la réponse, et la question est déjà tranchée par la sortie brute du 2026-08-04 (compte principal avec IBAN, trois pockets sans).
...
**Elle absorbe M1 et M2** *(venus de T31 le 2026-08-13, décision 50)* : *un produit Trade Republic inconnu ne fait plus échouer l'import entier* est le cas particulier de son critère (2), et *l'IBAN du PEA vient de son segment* celui de son critère (1). Deux tâches touchant le même fichier pour le même résultat, à deux places de l'ordre, c'était le travail fait deux fois et un risque de collision pour rien.

**Périmètre** *(corrigé le 2026-08-13, décision 47 — il était impossible à tenir)* : **`src/pdf-import.ts` et `src/pdf-import.test.ts`**. Le relevé rend ses segments tels qu'il les a trouvés, portant leur nom imprimé et leur IBAN quand il y est ; l'exigence des trois comptes disparaît ; **un segment non reconnu est compté et nommé**, jamais sauté. `accountKey` devient un repli interne. **Ne pas toucher** : le contrat de `src/server.ts`, `web/src/main.jsx`, `src/qualification-oracle.test.ts` (intouchable, décision 43), la reconnaissance de format et la lecture de mise en page — c'est la limite assumée.

> **Pourquoi le test entre dans le périmètre.** Mesuré : `src/pdf-import.test.ts` assère aujourd'hui que **chaque relevé LBP rend exactement `CCP, LIVRET_A, LIVRET_JEUNE`, dans cet ordre** — c'est-à-dire précisément l'exigence des trois comptes que cette tâche supprime. Avec l'ancien périmètre, Codex livrait le changement, voyait la suite rougir, et n'avait pas le droit de toucher le fichier qui rougit : le mur. Il n'avait alors que trois issues, toutes mauvaises — sortir du périmètre, rendre du rouge, ou défaire son propre changement.

**Critères** — (1) un relevé LBP amputé d'un de ses comptes s'importe quand même ; (2) un segment inconnu est nommé au lieu d'être sauté ; (3) *(chiffré le 2026-08-13, décision 48 — « les mêmes chiffres » ne disait pas lesquels)* **les sept chiffres de référence que `src/pdf-import.test.ts` assère déjà ne bougent pas d'une unité, et la sortie les montre** : La Banque Postale sur ses 12 relevés de référence — **CCP 299 mouvements, Livret A 45, Livret Jeune 6**, IBAN lus **CCP 12 · Livret A 11 · Livret Jeune 2** — et Nickel sur ses 9 relevés de référence, **54 mouvements** ; Trade Republic garde l'arithmétique déjà vérifiée par `verifyTradeRepublicTotals` ; (4) **sur chaque segment de chaque relevé, `ouverture + Σ mouvements = clôture`** — l'écart, s'il existe, est nommé avec son fichier et son compte.
...
**Périmètre** : `src/server.ts` — import en deux temps, **détecter puis associer**, et le premier temps a **sa route, nommée** *(décision 45, 2026-08-13)* : **`POST /imports/pdf/detect`**, même charge utile que l'import, **n'écrit rien**, rend pour chaque fichier l'établissement reconnu et ses segments avec leur nom imprimé, leur IBAN et le compte proposé. `POST /imports/pdf` garde son contrat, avec la correspondance que l'utilisateur a confirmée. `PdfAccountKey` sort du contrat public ; le rapprochement par IBAN est **borné aux comptes de l'établissement détecté**. `web/src/main.jsx` — `pdfAccountKeys` disparaît, le formulaire ne montre plus sept listes mais les comptes réellement trouvés dans les fichiers déposés ; **le navigateur garde les fichiers entre les deux appels**, il les a déjà. **Ne pas toucher** `src/pdf-import.ts` : T43a l'a fait.

> **Pourquoi nommer la route.** *« Import en deux temps »* était les trois seuls mots que Codex recevait sur le mécanisme central de la tâche. Mesuré : la route actuelle prend **un fichier** et une correspondance, et c'est le navigateur qui boucle sur le lot. « Détecter puis associer » voulait donc dire soit une route neuve, soit un contrat modifié — et selon le choix deviné, les 25 fichiers montent une fois ou deux.

**Critères** — (1) *(reformulé le 2026-08-13)* un import mensuel ne demande une correspondance que pour un compte **dont ni l'IBAN ni le nom imprimé ne rejoignent un compte connu de cet établissement** ; les autres se rapprochent seuls, Trade Republic compris dès le deuxième import ; (2) un rapprochement qui trouve **plusieurs** candidats demande au lieu de choisir ; (3) `git grep` ne rend aucun nom **désignant un compte réel** dans `web/` ni dans le contrat serveur (voir critère 7 du §8) ; (4) **le corpus s'importe par la route, et un test neuf le prouve** — `src/imports-pdf-corpus.test.ts`, fichier **nouveau**, dépose les 25 fichiers par `POST /imports/pdf/detect` puis `POST /imports/pdf` et retrouve les chiffres du critère (3) de T43a. *(Reformulé le 2026-08-13, décision 43 : la version précédente demandait de réécrire `src/qualification-oracle.test.ts`, qui ne se touche plus — c'est lui, le rejeu figé, et sa valeur de référence vient précisément de son immobilité.)*
...
> **Et sa recette se fait sur la base réelle, qu'on met de côté d'abord** *(décision 36, 2026-08-13 — remplace « sur une copie » ; **le vidage devient un renommage** le même jour, décision 44)*. C'est le seul moment du PRD où la base repart de zéro, et c'est ici parce que c'est ici que naît l'outil qui la reconstruit proprement. Ce qu'on écarte : 9 comptes homonymes sans IBAN, hérités de créations par `curl`. Ce qu'on reconstruit : chaque compte avec son nom, son IBAN quand il existe, son `display_name`.
>
> **On ne vide pas, on renomme — et c'est ce qui rend le geste réversible** *(décision 44, 2026-08-13)*. `data/gestio.db` devient `data/gestio-avant-onboarding.db` : l'application ne trouve plus de base, en fabrique une neuve, et le déclencheur voulu — tables vides — se produit exactement pareil. **Mesuré, et c'est la raison** : `scripts/` ne contient **aucun outil de restauration** — `restore.sh` est livré par **T36, quatre tâches plus loin** — et `backups/` porte **deux fichiers, tous deux datés du 2026-08-04**, alors que la base a été écrite après ; le cron de sauvegarde ne tourne pas. Un vidage littéral placerait donc le seul geste irréversible du PRD au seul moment où rien ne sait rattraper. Le renommage coûte zéro ligne de code, se défait en une commande, et l'ancienne base reste lisible avec la même clé. **On lance `npm run backup` juste avant**, pour avoir enfin une sauvegarde datée du jour.
...
**Deux noms sont déjà pris dans `web/src/main.jsx`, et il faut le dire** *(2026-08-13, décision 39)* : le composant **`Onboarding`** existe — c'est aujourd'hui l'écran « Connecter votre première banque » — et il est **remplacé** par la suite de questions, pas doublé d'un homonyme. **`BankConnect`** est **réutilisé tel quel** : il porte déjà la connexion, l'ouverture du navigateur et l'attente du retour ; c'est aussi lui qui porte le `defaultValue` « La Banque Postale » que le critère 7 du §8 attribue à cette tâche.

**Critère** — partir d'une **base vide**, déclarer ses banques, en connecter une, voir ses comptes apparaître **et pouvoir les distinguer** — sans terminal. *Recette* : c'est le critère, sur la base réelle mise de côté, dans le même geste que la rotation de clé — étapes 0 à 1 ; **et c'est ici qu'on renomme les quatre Revolut** (geste venu de T33, décision 40 : c'est cette tâche qui livre l'écran de nommage). *Préserve* : le déclenchement reste conditionné aux tables vides et à rien d'autre — **aucune livraison ne réinitialise une base d'elle-même**, la mise de côté est un geste humain ; l'ajout ordinaire reste atteignable après coup ; aucun solde n'est saisi.
...
**Ses prérequis, et ils sont durs — trois, nommés le 2026-08-13** : une base **onboardée** (T45b), le **corpus réimporté** sur cette base neuve, et une **synchro Revolut** (T34). La base doit contenir les mouvements que l'oracle décrit : l'oracle porte **911 mouvements** sur 10 comptes et 14 mois ; la base en portait **609** à la dernière mesure, parce que l'import complet du corpus n'a **jamais** réussi — l'étape 7 a bloqué deux fois.
...
**Périmètre** *(refondu le 2026-08-13, décisions 41 à 43 et 49)* : `src/db.ts` — la table d'oracle, additive, chiffrée donc sauvegardée. Elle référence **la ligne de transaction**, jamais l'empreinte (C11). Elle porte deux types de faits — *confirmé par relevé* et *nature tranchée* — avec leur date et leur preuve. `scripts/import-oracle.ts` — **fichier neuf**, lancé **une seule fois** : il lit l'artefact, attache, écrit le premier état et **imprime les deux compteurs**. `src/server.ts` — `DELETE /transactions/:id` **refuse** un mouvement qui porte une vérité. `src/oracle-import.test.ts` — **fichier neuf**. **Ne pas toucher `src/qualification-oracle.test.ts`** : il ne change pas d'une ligne, et c'est ce qui lui donne sa valeur de référence.

> **Un script, pas une route, et le repli de chemin reste** *(décision 41)*. L'artefact n'est **pas un canal du produit** : il est le point de départ, lu une fois. Après quoi l'oracle vit dans la base chiffrée et grandit tout seul — l'application n'a plus jamais besoin du fichier. Donc aucune route, aucun chemin permanent dans le code de production.
> **Et on ne supprime pas le repli WSL.** Mesuré le 2026-08-13 : `.env` porte cinq clés et **`GESTIO_T27_ORACLE_REPO` n'en fait pas partie** — c'est le repli qui trouve l'oracle aujourd'hui, y compris dans le rejeu vert de ce matin. Le supprimer rendrait la suite rouge partout, worktree **et** maître, jusqu'à ce qu'un humain édite un fichier que `.gitignore` empêche tout diff de porter. Le repli vise `/mnt/c/Users/<user>/Documents/releves-pdf`, où l'oracle reste même après la sortie du projet de `/mnt/c` (décision 14).
> **Ce qui devait disparaître, c'est le saut — et il est déjà attrapé.** Un `t.skip` ne fait plus passer personne : le critère 8 du §8 exige **zéro skip**, et il est désormais mesuré atteignable (40 tests · 40 pass · 0 skip). Source absente ⇒ skip ⇒ la livraison n'est pas VERTE, sans qu'on touche au test. **Aucun drapeau d'échappement** *(supprimé le 2026-08-13)* : un drapeau serait précisément le mécanisme qui refabrique un vert vide.

**Ses sept étapes, dans l'ordre** *(décision 49 — la tâche ne se coupe pas, mais son ordre ne se devine pas)* : (1) la table dans `src/db.ts` ; (2) le script qui lit l'artefact et **s'arrête en nommant** toute nature qu'il ne connaît pas ; (3) la correspondance des comptes, **fournie en donnée d'entrée**, jamais devinée ; (4) l'attachement par `deduplicateTransactions` ; (5) les deux compteurs imprimés ; (6) le refus de `DELETE` ; (7) le test neuf. Une seule recette, à la fin.

> **Pourquoi on ne la coupe pas, alors qu'elle demande huit gestes.** La coupure évidente — *« la table et le refus de suppression »* d'un côté, *« le premier état entre »* de l'autre — ne marche pas : pour prouver qu'on refuse de supprimer un mouvement qui porte une vérité, **il faut une vérité en base**, donc l'import. La première moitié n'aurait aucune recette jouable, et on refabriquerait le défaut qu'on vient de corriger quatre fois dans ce PRD.

**Le vocabulaire des natures s'unifie ; on ne pose aucune couche de traduction** *(décision 30, 2026-08-13)*. Mesuré : l'oracle écrit `virement_interne` (382), `externe_probable` (127), `virement_a_verifier` (97) ; `src/schema.ts` connaît `virement_intercompte`, `virement_externe`, `virement_a_verifier`. Deux mots sur trois diffèrent — recopiés tels quels, ils sortiraient de l'énumération, le `PATCH` de T35 les rejetterait et la recette de T31 ne les compterait jamais. **Le code fait foi, comme pour F2 et F3** : le renommage (`virement_interne → virement_intercompte`, `externe_probable → virement_externe`) se fait **à l'import, dans le script**, jamais dans le schéma — les 606 décisions et les 191 paires ne bougent pas d'un chiffre, la base ne migre pas, l'application ne change pas d'une ligne. Rien ne se perd : la nuance « probable » est déjà portée par `confidence`, dont le décompte `probable = 127` égale exactement celui d'`externe_probable`. **La garde qui rend l'unification vraie au lieu d'espérée : le script s'arrête et nomme toute nature absente de `transactionNatures`.**

> **L'artefact ne se réécrit pas** *(décision 43, 2026-08-13 — amende la 30)*. La 30 demandait de renommer **dans le fichier**, pour éviter « une couche de traduction ». Deux mesures la retournent : cette table de traduction **existe déjà**, à trois entrées, dans `src/qualification-oracle.test.ts` — elle n'est donc pas à poser, elle est à réutiliser ; et le fichier vit dans un dépôt **hors git, sans remote, hors sauvegarde**, dont le manifeste ne hache que les 26 documents, pas lui. L'éditer ferait cesser d'être vrai *« l'oracle T27, figé le 2026-08-10 »*, sans laisser la moindre trace si la manipulation dérape. **L'intention de la 30 est tenue à la lettre — aucune nature bâtarde en base, un seul point de traduction — et l'oracle reste un artefact qu'on lit sans jamais l'écrire.**
...
**Critères** *(1 et 2 refondus le 2026-08-13)* — (1) le premier état attache **380 décisions sur 380** — celles nées des relevés : 246 La Banque Postale, 44 Nickel, 90 Trade Republic — et **191 paires sur 191** ; **un seul non attaché est ROUGE** ; (2) **le taux d'attachement des 226 décisions Revolut est mesuré et écrit** — un nombre, pas une porte : elles s'appuient sur des lignes venues de l'API quand l'oracle les a lues dans un CSV, et ce chiffre n'existe nulle part aujourd'hui ; (3) les quatre niveaux de `confidence` sont en base à l'identique — 336 / 46 / 127 / 97 ; (4) **aucune nature hors `transactionNatures` n'entre en base** : le script traduit les trois mots de l'artefact et **s'arrête en nommant** tout mot qu'il ne connaît pas *(décision 43 : la garde vit dans le script, seul lecteur du fichier, et non dans le test figé)* ; (5) la source retirée, la suite **ne peut pas être verte** — le rejeu se saute, et un skip n'est pas VERT par le critère 8 du §8 ; (6) `DELETE /transactions/:id` refuse un mouvement qui porte une vérité, et dit pourquoi.
...
*Recette (précisée le 2026-08-13, décision 42)* : sur la base reconstruite (T45b), corpus réimporté et Revolut resynchronisé, lancer le script d'import du premier état et **lire les deux compteurs qu'il imprime** — `380/380` attachées, et le taux Revolut `x/226`. Puis **relancer le script en lecture** : il rouvre la base, recompte, réimprime les deux mêmes chiffres — c'est la preuve que le premier état a bien été écrit. Enfin, dans l'application, tenter de supprimer un mouvement qui porte une vérité et lire le refus. *(La formulation précédente — « lire les deux compteurs, redémarrer l'application, les relire » — demandait un écran d'oracle, que le §4 interdit : « l'oracle est un compteur et un vérificateur, il ne s'affiche pas ». Les deux phrases ensemble ne laissaient à Codex que deux livraisons, toutes deux fausses.)* *Préserve* : le rejeu du test rend **exactement** 191 paires et 606 décisions — ce nombre-là ne bouge pas ; aucun libellé ni IBAN ne sort de la base ; aucune transaction n'est modifiée par l'import de l'oracle.
...
**Critères** — (1) corriger une nature, réimporter le même relevé, relancer une synchro : **la correction survit**, et l'écran dit d'où vient chaque décision ; (2) **le décompte d'oracle augmente d'un après une décision tranchée** *(critère venu de T44, qui n'avait pas le geste pour le prouver)*. *Recette (complétée le 2026-08-13, décision 40)* : contester une qualification, lire le compteur monter, puis la voir survivre à un ré-import — étape 8. **Et, sur le même écran, lire enfin le décompte des trois natures** : combien de mouvements portent `virement_intercompte`, combien `virement_a_verifier`, combien `virement_externe`. *Ce geste vient de T31, où il était injouable* — aucun écran ne comptait par nature, et la liste des mouvements s'arrête à 100 lignes ; c'est cette tâche qui livre l'écran qui les montre. **Ce chiffre n'a jamais été vu** (étape 8) : la recette produit donc en prime la mesure qui manque au lot C. *Préserve* : toute transaction non tranchée continue d'être requalifiée ; `POST /transactions/resolve` et `DELETE /transactions/:id` gardent leur sémantique de doublon ; l'oracle rejoue à l'identique et **ne rétrécit jamais**.
...
```
… ▸ T45a ▸ T45b ▸ [ npm run backup + `data/gestio.db` → `data/gestio-avant-onboarding.db`
   │              + rotation de la clé + 3 ré-authentifications
   │              corpus réimporté (25 fichiers) + Revolut resynchronisé ]  ← humain, une fois
   ↓
T34 ▸ T44 ▸ …
```

C'est le seul moment où la base repart de zéro, et il est ici parce que c'est ici que naît l'outil qui la reconstruit. Les trois authentifications fortes sont dues de toute façon — la rotation de clé du lot 0 invalide les sessions. Ce qui s'écarte : 9 comptes homonymes sans IBAN. Ce qui se gagne : la matière de T44 dans un état propre, et `display_name` posé par l'onboarding au lieu d'être tapé à la main.

**On renomme, on ne vide pas** *(décision 44, 2026-08-13)*. `restore.sh` n'arrive qu'à **T36**, quatre tâches plus loin, et la dernière sauvegarde date du **2026-08-04** : un vidage littéral placerait le seul geste irréversible du PRD exactement là où rien ne sait rattraper. Le renommage produit le même déclencheur — tables vides — coûte zéro ligne de code, et se défait en une commande.
...
4. **Trois compteurs, trois noms — le mot « oracle » cessait d'en désigner un seul** *(refondu le 2026-08-13, décision 32)*. Confondus, ils produisent le mode de panne du §1 : un total qui a l'air juste.
   - **Le rejeu** — 191 paires, **606 décisions**, rejouées en mémoire par `qualification-oracle.test.ts` dans une base neuve, chemin CSV compris. Invariant, sur tout le PRD, et c'est la porte dure : ROUGE si la source manque, ROUGE si le test se saute. C'est ce que vise chaque *Préserve* qui dit « l'oracle rejoue à l'identique ». **Mesuré vert le 2026-08-13, pour la première fois** — 3,9 s, `.env` chargé : la ligne de base existe, et ce fichier de test **ne se modifie plus** (décision 43).
...
5. **Les 18 méthodes ont un état connu**, chacune avec le geste qui l'a exercée. *(Recomptées le 2026-08-13 dans `src/server.ts` : **18 routes déclarées exactement**, aucun `PUT`, aucun `PATCH` — le chiffre du critère est vérifiable, il ne vient pas d'un document.)* Une méthode sans appelant est branchée ou supprimée. *(`POST /transactions` n'est plus candidate à la suppression : T48 lui donne son usage, la saisie des dépenses en espèces.)*
...
**Rendues le 2026-08-13, au terme de la revue d'exécutabilité et de jugeabilité** *(session `/grilling`, la troisième et la dernière, trois rounds, quinze mesures neuves ; aucune des trente-sept décisions ci-dessus rouverte, deux amendées — la 30 et la 36)* :

38. **Le rejeu de l'oracle a été exécuté, et il est vert.** `40 tests · 40 pass · 0 skip · 11,2 s` avec `--env-file=.env`, le rejeu seul en 3,9 s. Huit tâches promettaient de préserver un résultat que personne n'avait jamais vu ; **la ligne de base existe désormais, datée**. *Amende le §5 et le critère 8.*
39. **Le critère de T31 ne se satisfait plus d'un effacement, et la tâche reçoit enfin un périmètre.** Le débordement touche **24 mouvements sur les 13 relevés**, dans `qualification_label` autant que dans `label`, et **20 portent une des 606 décisions**. Ce qui prouve la coupe, c'est le rejeu resté à 191/606 — simulé vert le même jour. T31 était la seule tâche du lot sans périmètre ni « ne pas toucher ».
40. **Une recette ne demande que des gestes qui existent le jour où on la joue.** Trois déménagent sans qu'aucune tâche ne bouge : le décompte des trois natures **T31 → T35** (aucun écran ne compte, la liste s'arrête à 100 lignes), le renommage des quatre Revolut **T33 → T45a** (aucun écran ne renomme). C'est le même défaut que les deux inversions de la session 2, mais dans la recette au lieu de l'ordre.
41. **L'oracle entre par un script lancé une fois — `scripts/import-oracle.ts` — et le repli de chemin reste.** L'artefact n'est pas un canal du produit : il est un point de départ. Et `GESTIO_T27_ORACLE_REPO` n'est **pas** dans `.env` : supprimer le repli rendait la suite rouge partout, sans qu'aucun diff puisse la réparer. Ce qui disparaît, c'est le saut — déjà attrapé par « zéro skip ».
42. **Les deux compteurs sont imprimés par le script, jamais affichés.** La recette de T44 demandait de les lire à l'écran quand le §4 interdit à l'oracle de s'afficher. On relance le script en lecture : il recompte et réimprime — la persistance est prouvée, aucun écran d'oracle n'existe.
43. **`src/qualification-oracle.test.ts` ne se touche plus.** *Amende la 30.* Sa valeur de référence vient de son immobilité. Les deux gardes qui demandaient de le réécrire déménagent : la nature inconnue va dans le **script** (seul lecteur de l'artefact), l'épreuve de la route va dans un **fichier de test neuf**. Et **l'artefact ne se réécrit pas** : la table de traduction existe déjà dans ce test, et le fichier vit hors git, hors sauvegarde, hors manifeste.
44. **On ne vide pas la base réelle, on la renomme, après sauvegarde.** *Amende la 36.* `restore.sh` arrive à T36, quatre tâches plus loin, et `backups/` porte deux fichiers datés du 2026-08-04 : le seul geste irréversible du PRD tombait là où rien ne sait rattraper. Le renommage produit le même déclencheur — tables vides — pour zéro ligne de code.
45. **La détection de T43b a sa route : `POST /imports/pdf/detect`.** Trois mots — « import en deux temps » — décrivaient le mécanisme central de la tâche ; selon ce que Codex devinait, les 25 fichiers montaient une fois ou deux.
46. **T34 : les connexions s'établissent sur le PC, la relance se fait depuis le téléphone.** La recette demandait ce que la tâche déclare elle-même hors périmètre (C4), et l'échec serait tombé **après** l'authentification forte.
47. **Le périmètre de T43a inclut `src/pdf-import.test.ts`.** Ce fichier assère aujourd'hui que chaque relevé LBP rend exactement ses trois comptes — l'exigence même que la tâche supprime. L'ancien périmètre laissait Codex face à trois issues, toutes mauvaises.
48. **Le critère (3) de T43a nomme ses chiffres** : CCP 299, Livret A 45, Livret Jeune 6, IBAN 12/11/2, Nickel 54, sur les relevés de référence que le test porte déjà. « Les mêmes chiffres » ne disait pas lesquels, et un juge n'avait rien à comparer.
49. **T44 ne se coupe pas, mais ses sept étapes sont écrites dans l'ordre.** La coupure évidente laisserait sa première moitié sans recette jouable : prouver le refus de suppression exige une vérité en base, donc l'import.

50. **Les cinq points hérités de la review T27 sont retrouvés et arbitrés.** `F5` et `F6` n'étaient définis **nulle part sur le disque** — seulement dans `.lamoms/prd-annexes.md`, archivé : Codex les recevait comme deux sigles vides. **M1 et M2 partent dans T43a** (c'est son sujet mot pour mot), **M3 tombe** (il répare des lignes que la reconstruction de T45a écarte), **F5 sort** (son cas réel n'a jamais été observé, l'étape 8 n'ayant jamais été atteinte — il redevient un point de récolte), **F6 reste**. T31 se referme sur **un défaut de parseur, un message d'erreur et une ligne d'outillage**.
