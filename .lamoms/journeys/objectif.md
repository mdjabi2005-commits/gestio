...
journey:
  ...
  scope_in:
    ...
    - "Présenter les avoirs financiers mobilisables pour l'objectif, notamment comptes d'épargne, portefeuille crypto ou autres supports pertinents, en excluant le fonds d'urgence."
    - "Permettre à l'utilisateur de choisir quelle part de ses avoirs mobilisables il affecte déjà à l'objectif."
    ...
    - "Sans échéance, estimer un délai réaliste à partir de la capacité d'épargne actuelle."
    ...
    - "Proposer une poursuite volontaire vers simulation ou ajustement lorsque l'utilisateur souhaite explorer d'autres paramètres ou rechercher des marges d'action."
  scope_out:
    ...
    - "Utiliser le fonds d'urgence comme somme librement mobilisable pour un objectif."
    - "Considérer un objectif incompatible avec les paramètres actuels comme impossible ou interdit."
    - "Simuler dans ce parcours plusieurs variantes de montant, échéance, effort ou somme initiale sans modifier l'objectif de référence."
    - "Déterminer concrètement quelles dépenses ou pockets doivent être ajustées pour augmenter la capacité d'épargne."
...
user_flow:
  ...
  steps:
    ...
    - action: "L'utilisateur consulte les avoirs déjà disponibles susceptibles de contribuer à l'objectif."
      visible_result: "Gestio présente les comptes d'épargne, portefeuilles crypto et autres avoirs pertinents qu'il connaît comme potentiellement mobilisables, tout en excluant explicitement les comptes réservés au fonds d'urgence."
    ...
    - action: "L'utilisateur choisit les avoirs ou montants qu'il souhaite effectivement affecter dès maintenant à cet objectif."
      visible_result: "Gestio distingue les avoirs potentiellement mobilisables de la somme réellement affectée à l'objectif par décision de l'utilisateur."
    ...
    - action: "Si aucune échéance n'est fixée, l'utilisateur demande à Gestio ce que sa situation actuelle permet."
      visible_result: "Gestio estime un délai réaliste à partir du montant restant à financer et de la capacité d'épargne actuelle, en distinguant cette projection des faits observés."
    ...
    - action: "L'utilisateur décide de la suite à donner à son objectif."
      visible_result: "Il peut conserver l'objectif tel quel, y compris lorsqu'un écart existe, modifier directement l'objectif de référence, explorer une simulation, rechercher des ajustements ou quitter le parcours. Gestio ne choisit pas à sa place."
      possible_error: "Aucun parcours complémentaire pertinent n'est disponible ; l'objectif peut néanmoins être conservé avec son état d'évaluation courant."
      state: expected
  ...
  exit_conditions:
    ...
    - "L'utilisateur poursuit vers ajustement pour rechercher concrètement des marges permettant d'augmenter sa capacité d'épargne."
...
journey_links:
  ...
  - "simulation: permet d'explorer des variantes sans modifier immédiatement l'objectif de référence"
  - "ajustement: permet de rechercher concrètement comment augmenter la capacité d'épargne lorsqu'un écart existe"
code_refs: []
evidence_refs: []
status: draft
...
- La relation de travail actuelle entre les grandeurs financières est `SH = SB + SPP + CP`, avec `SH` considéré comme fixé par la situation observée et `CP` comme la part restante après `SB` et `SPP`. Ce vocabulaire reste à normaliser après stabilisation des parcours.
...
- Les avoirs mobilisables peuvent inclure des comptes d'épargne, portefeuilles crypto ou autres supports pertinents, mais le fonds d'urgence est exclu de l'argent librement mobilisable pour les objectifs.
- Un avoir mobilisable n'est pas automatiquement affecté à l'objectif : l'utilisateur choisit ce qu'il souhaite réellement y consacrer.
...
- La simulation explore des hypothèses sans modifier immédiatement l'objectif de référence ; l'ajustement recherche concrètement où agir pour modifier la capacité d'épargne.
