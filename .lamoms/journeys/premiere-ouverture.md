...
journey:
  ...
  expected_outcome: "L'utilisateur obtient une première situation financière exploitable, comprenant sa situation sur la période choisie, son rythme financier et une estimation de sa capacité financière, puis peut entrer dans l'usage courant."
  ...
  scope_in:
    ...
    - "Analyser le rythme financier observé."
    - "Calculer une première capacité financière."
...
user_flow:
  ...
  steps:
    ...
    - action: "L'utilisateur indique quels comptes d'épargne constituent son fonds d'urgence."
      visible_result: "Gestio distingue les comptes réservés au fonds d'urgence des autres avoirs mobilisables. Le rôle est attribué au niveau du compte, pas d'un montant arbitraire dans un compte."
    ...
    - action: "L'utilisateur consulte son rythme financier."
      visible_result: "Gestio affiche ce qui est habituellement gagné, dépensé et épargné par mois à partir de l'historique disponible, en distinguant l'observation historique de la situation du mois courant."
      ...
      state: expected
    - action: "L'utilisateur consulte sa capacité financière."
      visible_result: "Gestio affiche une estimation de la somme pouvant être épargnée de manière soutenable par mois, distincte de l'épargne effectivement observée dans l'historique."
      possible_error: "La capacité ne peut pas être estimée de façon suffisamment fiable ; Gestio doit expliquer quelles données ou qualifications manquent."
      state: expected
  success_result: "Une première situation financière exploitable est construite. L'utilisateur comprend où il en est sur une vue mensuelle ou annuelle, son rythme financier historique, sa capacité financière estimée, la répartition de ses dépenses par pockets et la protection apportée par son fonds d'urgence. Il peut ensuite entrer dans l'usage courant sans être obligé de créer un objectif."
  exit_conditions:
...
- Le **rythme financier** correspond à ce qui est habituellement gagné, dépensé et épargné par mois à partir de l'historique.
- La **capacité financière** correspond à une estimation de ce qui peut être épargné de manière soutenable chaque mois. Elle est distincte de l'épargne effectivement observée.
