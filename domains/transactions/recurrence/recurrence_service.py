...
def backfill_recurrences_to_today(db_path: Optional[str] = None) -> None:
    ...
    Wrapper simplifié pour backfill_all_recurrences(). Utilise la nouvelle table 
    recurrences et génère des transactions avec source='récurrente_auto'.
    
    Args:
        db_path: Chemin optionnel de la base de données (backward compatibility)
...
def sync_recurrences_to_echeances() -> int:
    ...
    for rec in recurrences:
        ...
        while current <= fin_mois_suivant:
            ...
            # Avancer selon la fréquence via mapping unifié
            delta = FREQ_DELTAS.get(frequence.lower())
            if not delta:
