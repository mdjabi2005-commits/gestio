...
2.  **Traitement OCR (Optionnel)**: Si un fichier est téléversé, `OCRService.process_ticket` l'analyse et retourne un objet `Transaction` pour pré-remplir le formulaire.
3.  **Validation Utilisateur**: L'utilisateur revoit/modifie le formulaire. À la soumission, les données sont validées (champs requis, types).
4.  **Persistance Transaction**: Les données validées (en `dict`) sont envoyées à `TransactionRepository.add_transaction`. Cela insère une ligne dans la table `transactions` et retourne le nouvel `id`.
