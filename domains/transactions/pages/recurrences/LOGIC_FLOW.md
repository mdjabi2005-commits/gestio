...
1.  **Chargement de Données**: La page récupère toutes les définitions de récurrence actives depuis `RecurrenceRepository` sous forme de `List[Recurrence]`.
2.  **Moteur de Projection**: Pour chaque objet récurrence, la méthode `generate_occurrences(current_date)` est appelée. Cela calcule toutes les dates futures (ex: 12 prochains mois) basées sur la fréquence (mensuelle, annuelle).
3.  **Transformation de Données**: La liste résultante d'occurrences virtuelles (dictionnaires) est convertie en un `pd.DataFrame`.
...
5.  **Flux de Création**: Les nouvelles récurrences sont créées via un formulaire, instanciées comme objets `Recurrence`, et persistées via `RecurrenceRepository.add_recurrence`.
