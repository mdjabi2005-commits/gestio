...
import logging
import pandas as pd
from domains.transactions.database import TRANSACTION_TYPES, TRANSACTION_CATEGORIES
from shared.ui.toast_components import toast_success
...
def render_transaction_table(filtered_df, transaction_repository):
    ...
    result = st.data_editor(
        ...
        column_order=["Supprimer", "date", "type", "category", "subcategory", "amount", "description", "Pieces Jointes"],
        hide_index=True,
            ...
            if uploaded_files:
                if st.button("Envoyer", type="primary", key=f"send_{selected_tx_id}"):
                    ...
                    for f in uploaded_files:
                        if attachment_service.add_attachment(selected_tx_id, f, f.name):
                            success_count += 1
