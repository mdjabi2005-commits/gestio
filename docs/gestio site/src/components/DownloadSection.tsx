      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {platforms.map((p, i) => (
          <Reveal key={p.name} delay={i * 100}>
            <div className="bg-card border border-border rounded-2xl p-10 text-center transition-all hover:-translate-y-2 hover:border-primary hover:shadow-[0_24px_48px_rgba(0,0,0,0.3)]">
              <div className="w-[72px] h-[72px] mx-auto mb-6 bg-primary/10 rounded-2xl flex items-center justify-center">
                {p.icon}
              </div>
              ...
              <a
                href={`#download-${p.name.toLowerCase()}`}
                className="inline-flex items-center justify-center gap-2.5 w-full bg-gradient-primary text-primary-foreground px-8 py-4 rounded-xl font-semibold shadow-primary hover:-translate-y-1 hover:shadow-primary-hover transition-all no-underline"
              >
