...
def _preload_ocr() -> None:
    """Précharge OCRService singleton en arrière-plan pour éliminer le cold start PyInstaller."""
    try:
        from domains.transactions.ocr.services.ocr_service import get_ocr_service
        get_ocr_service()
        logger.info("OCRService préchargé en arrière-plan ✅")
    except Exception as e:
        logger.warning(f"Préchargement OCR échoué (non bloquant) : {e}", exc_info=True)
